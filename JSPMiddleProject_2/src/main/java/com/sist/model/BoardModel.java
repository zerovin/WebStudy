...
//요청처리 후 결과값 JSP로 전송 => Model
//오라클 연결(DAO), 데이터 모음(VO), 전송(Model) => Model Back-end
/*
 *  1.GIT
 *  2.데이터베이스 설계 => 정규화
 *  3.ERD
...
/*
 *  JSP => Java : URL 주소로 Java 호출 불가
 *                request 객체 전송
 *  JSP => JSP : URL주소로 값 전송 가능
 *  JSP => Servlet : URL주소로 값 전송 가능
 */
public class BoardModel {
	...
	public void boardInsertOk(HttpServletRequest request, HttpServletResponse response) {
		try {
			request.setCharacterEncoding("UTF-8"); //예외처리 필요
		}catch(Exception ex) {}
		...
		//화면 이동
		try {
			response.sendRedirect("list.jsp"); //예외처리 필요			
		}catch(Exception ex) {}
	}
